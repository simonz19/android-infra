 ...
 * 
 */
public class BorderImageButton extends AppCompatImageButton implements IBadgeView, IBorderView, IHintView {
	...
	public BorderImageButton(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init(Context context, AttributeSet attrs, int defStyle) {
		TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.BorderableViews,
				defStyle, 0);
		...
		for (int i = 0; i < n; i++) {
			...
			if (attr == R.styleable.BorderableViews_borders) {
				...
				borderInsidePadding = attributes.getBoolean(attr, borderInsidePadding);
			}
		}
		attributes.recycle();
		if (topDrawable != null) {
		...
		attributes = context.obtainStyledAttributes(attrs, R.styleable.Badge, defStyle, 0);
		...
		attributes = context.obtainStyledAttributes(attrs, R.styleable.Hint, defStyle, 0);
		for(int i=0;i<n;i++){
			int attr=attributes.getIndex(i);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (borderLeftColorList != null || borderLeftDrawable != null) {
			...
			if (borderLeftColorList != null) {
				ColorDrawableHelper.drawRect(canvas, l, t, r, b, getColorPaint(),
						borderLeftColorList.getColorForState(getDrawableState(),
		...
		if (borderRightColorList != null || borderRightDrawable != null) {
			...
			if (borderRightColorList != null) {
				ColorDrawableHelper.drawRect(canvas, l, t, r, b, getColorPaint(),
						borderRightColorList.getColorForState(getDrawableState(),
		...
		if (borderTopColorList != null || borderTopDrawable != null) {
			...
			if (borderTopColorList != null) {
				ColorDrawableHelper.drawRect(
						...
						b,
						getColorPaint(),
						borderTopColorList.getColorForState(getDrawableState(),
		...
		if (borderBottomColorList != null || borderBottomDrawable != null) {
			...
			if (borderBottomColorList != null) {
				ColorDrawableHelper.drawRect(canvas, l, t, r, b, getColorPaint(),
						borderBottomColorList.getColorForState(getDrawableState(),
								borderBottomColorList.getDefaultColor()));
			}
			if (borderBottomDrawable != null) {
				int bheight = borderBottomSize;
				if (bheight <= 0 && borderBottomDrawable.getIntrinsicHeight() > 0) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (badgeRadius > 0) {
			...
			badgeCx = (badgeBL || badgeTL) ? badgePadding : (badgeBR | badgeTR) ? width
					- badgePadding : 0;
			badgeCy = (badgeTL || badgeTR) ? badgePadding : (badgeBL | badgeBR) ? height
					- badgePadding : 0;
		}
		if(hintRadius>0){
			int width=getMeasuredWidth();
			int height=getMeasuredHeight();
			hintCx = (hintBL || hintTL) ? hintPadding : (hintBR | hintTR) ? width - hintPadding : 0;
			hintCy = (hintTL || hintTR) ? hintPadding : (hintBL | hintBR) ? height - hintPadding
					: 0;
		}
	}
	...
	//预留
	/*private void isPaddingTopAdd(){
		if(badgePaddingTopAdd==0){
			return ;
		}
		if (badgeTL || badgeTR) {
			badgeCy = badgeCy - badgePaddingTopAdd;
