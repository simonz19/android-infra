 ...
 * 
 */
public class BorderLinearLayout extends LinearLayout implements IBadgeView,
		IBorderView {
	...
	public BorderLinearLayout(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs);
	...
	private void init(Context context, AttributeSet attrs, int defStyle) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (attr == R.styleable.Badge_badgePosition) {
				...
				setBadgeColor(attributes.getColor(attr, 0x333333));
	...
	protected void onDraw(Canvas canvas) {
		...
		if (borderTopColorList != null || borderTopDrawable != null) {
			int l = left;
			int t = getMeasuredHeight() - borderTopSize;
			int r = getMeasuredWidth() - right;
			int b = getMeasuredHeight() - bottom;
